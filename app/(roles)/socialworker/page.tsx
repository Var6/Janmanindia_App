...
export default async function SocialWorkerDashboard() {
  ...
  const [sw, pendingVerifications, openSos, activeCases, pendingInvoices] =
    await Promise.all([
      User.findById(session.id).lean(),
      User.find({ role: "user", "citizenProfile.verificationStatus": "pending" })
        .limit(10)
        .lean(),
      SosAlert.find({ status: "open" }).limit(10).lean(),
      Case.find({ socialWorker: session.id, status: "Open" }).limit(10).lean(),
      EodReport.find({ submittedBy: session.id, invoiceStatus: "pending" })
        .sort({ createdAt: -1 })
        .limit(5)
        .lean(),
    ]);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text)]">Social Worker Dashboard</h1>
      ...
      <section className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] overflow-hidden">
        ...
        {pendingVerifications.length === 0 ? (
          <p className="px-6 py-6 text-sm text-[var(--muted)] text-center">No pending verifications.</p>
        ) : (
          <div className="divide-y divide-[var(--border)]">
            {pendingVerifications.map((u) => (
              <div key={String(u._id)} className="px-6 py-4 flex items-center justify-between gap-4">
                <div>
                  <p className="font-medium text-sm text-[var(--text)]">{u.name}</p>
                  <p className="text-xs text-[var(--muted)]">{u.email}</p>
                  <p className="text-xs text-[var(--muted)] mt-0.5">
                    ID Type: {u.citizenProfile?.govtIdType ?? "—"}
                  </p>
                </div>
                <div className="flex gap-2">
                  {u.citizenProfile?.govtIdUrl && (
                    <a
                      href={u.citizenProfile.govtIdUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-xs px-3 py-1.5 rounded-lg border border-[var(--border)] hover:border-[var(--accent)] text-[var(--text)] transition-colors"
                    >
                      View ID
                    </a>
                  )}
      ...
      <section className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] overflow-hidden">
        <div className="px-6 py-4 border-b border-[var(--border)] flex items-center justify-between">
          <h2 className="font-semibold text-[var(--text)]">
            Open SOS Alerts
            {openSos.length > 0 && (
              <span className="ml-2 text-xs bg-orange-500 text-white rounded-full px-2 py-0.5">
                {openSos.length}
              </span>
            )}
          </h2>
          <Link href="/socialworker/escalate" className="text-xs text-[var(--accent)] hover:underline">
            Manage
          </Link>
        </div>
        {openSos.length === 0 ? (
          <p className="px-6 py-6 text-sm text-[var(--muted)] text-center">No open SOS alerts.</p>
        ) : (
          <div className="divide-y divide-[var(--border)]">
            {openSos.map((sos) => (
              <div key={String(sos._id)} className="px-6 py-4">
                <p className="text-sm font-medium text-[var(--text)]">{sos.description}</p>
                <p className="text-xs text-[var(--muted)] mt-0.5">
                  Location: {sos.location} &middot;{" "}
                  {new Date((sos as unknown as { createdAt: Date }).createdAt).toLocaleDateString("en-IN")}
                </p>
              </div>
            ))}
          </div>
      ...
      <section className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] overflow-hidden">
        <div className="px-6 py-4 border-b border-[var(--border)] flex items-center justify-between">
          <h2 className="font-semibold text-[var(--text)]">My Active Cases</h2>
          <Link href="/socialworker/cases" className="text-xs text-[var(--accent)] hover:underline">
            All cases
          </Link>
        </div>
        {activeCases.length === 0 ? (
          <p className="px-6 py-6 text-sm text-[var(--muted)] text-center">No active cases assigned.</p>
        ) : (
          <div className="divide-y divide-[var(--border)]">
            {activeCases.map((c) => (
