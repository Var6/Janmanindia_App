...
export default async function UserDashboard() {
  ...
  if (dbOk && mongoose.Types.ObjectId.isValid(session.id)) {
    ...
    [cases, appointments] = await Promise.all([
      Case.find({ citizen: userId }).sort({ updatedAt: -1 }).limit(10).lean(),
      Appointment.find({ citizen: userId }).sort({ requestedAt: -1 }).limit(5).lean(),
    ]);
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-(--muted) mt-1">Community Dashboard — JanmanIndia Legal Aid</p>
      </div>

      {/* Quick Actions */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Link href="/community/file-case" className="flex items-center gap-4 p-5 rounded-2xl bg-(--surface) border border-(--border) hover:border-(--accent) transition-colors group">
        ...
        <Link href="/community/sos" className="flex items-center gap-4 p-5 rounded-2xl bg-red-50 border border-red-200 hover:border-red-400 transition-colors">
        ...
        <Link href="/community/appointments" className="flex items-center gap-4 p-5 rounded-2xl bg-(--surface) border border-(--border) hover:border-(--accent) transition-colors">
      ...
      <section className="bg-(--surface) rounded-2xl border border-(--border) overflow-hidden">
        <div className="px-6 py-4 border-b border-(--border) flex items-center justify-between">
          ...
          <Link href="/community/case-tracker" className="text-xs text-(--accent) hover:underline">View all</Link>
        </div>
        {cases.length === 0 ? (
          <div className="px-6 py-8 text-center text-(--muted) text-sm">
            {dbOk
              ? <><span>No cases filed yet. </span><Link href="/community/file-case" className="text-(--accent) hover:underline">File your first case.</Link></>
      ...
      <section className="bg-(--surface) rounded-2xl border border-(--border) overflow-hidden">
        <div className="px-6 py-4 border-b border-(--border) flex items-center justify-between">
          ...
          <Link href="/community/appointments" className="text-xs text-(--accent) hover:underline">Manage</Link>
        </div>
