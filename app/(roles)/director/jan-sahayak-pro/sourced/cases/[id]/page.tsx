  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-(--muted) mt-1">{c.client} v. {c.respondent}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="bg-(--surface) border border-(--border) rounded-2xl p-5">
          <h3 className="text-sm font-bold text-(--text) mb-3">Metadata</h3>
          <dl className="grid grid-cols-[120px_1fr] gap-y-2 text-xs">
            ...
            <dt className="text-(--muted)">FIR No.</dt>   <dd className="text-(--text)">{c.firNo || "—"}</dd>
            <dt className="text-(--muted)">Section</dt>   <dd className="text-(--text)">{c.section || "—"}</dd>
            <dt className="text-(--muted)">PS</dt>        <dd className="text-(--text)">{c.ps || "—"}</dd>
            ...
            <dt className="text-(--muted)">Worker</dt>    <dd className="text-(--text)">{c.worker}</dd>
            <dt className="text-(--muted)">Created</dt>   <dd className="text-(--text)">{c.createdAt}</dd>
            <dt className="text-(--muted)">Updated</dt>   <dd className="text-(--text)">{c.updatedAt}</dd>
          </dl>
        </section>

        <section className="bg-(--surface) border border-(--border) rounded-2xl p-5 space-y-4">
          <div>
            <h3 className="text-sm font-bold text-(--text) mb-2">Facts</h3>
            <p className="text-xs text-(--text) whitespace-pre-line">{c.facts || "—"}</p>
          </div>
          <div>
            <h3 className="text-sm font-bold text-(--text) mb-2">Grounds</h3>
            <p className="text-xs text-(--text) whitespace-pre-line">{c.grounds || "—"}</p>
          </div>
          <div>
            <h3 className="text-sm font-bold text-(--text) mb-2">Relief</h3>
      ...
      <section className="bg-(--surface) border border-(--border) rounded-2xl p-5">
        <h3 className="text-sm font-bold text-(--text) mb-3">Hearings ({c.hearings.length})</h3>
        {c.hearings.length === 0 ? (
          <p className="text-xs text-(--muted)">No hearings logged.</p>
        ) : (
          <ul className="space-y-3">
            {c.hearings.map((h) => (
              <li key={h.id} className="border-l-2 border-(--accent) pl-3">
                ...
                {h.nextDate && <div className="text-[11px] text-(--accent) mt-1">Next: {h.nextDate}</div>}
                <div className="text-[10px] text-(--muted) mt-1">by {h.by}</div>
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="bg-(--surface) border border-(--border) rounded-2xl p-5">
        <h3 className="text-sm font-bold text-(--text) mb-3">Case Diary ({c.diary.length})</h3>
        {c.diary.length === 0 ? (
          <p className="text-xs text-(--muted)">No diary entries.</p>
        ) : (
          <ul className="space-y-3">
            {c.diary.map((d) => (
              <li key={d.id} className="border-l-2 border-(--border) pl-3">
                ...
                <div className="text-[10px] text-(--muted) mt-1">by {d.by}</div>
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="bg-(--surface) border border-(--border) rounded-2xl p-5">
        <h3 className="text-sm font-bold text-(--text) mb-3">Documents ({c.documents.length})</h3>
