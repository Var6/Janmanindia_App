    ...
    <div className="space-y-8">
      ...
      <form onSubmit={handleAuto} className="bg-(surface) rounded-2xl border border-(border) p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-(text) mb-1.5">{t("Case Location")} <span className="text-red-500">*</span></label>
            <input
              value={location}
              ...
              <div key={l._id} className="flex items-center justify-between px-5 py-3">
                <div>
                  ...
                  <p className="text-xs text-(muted)">
                    {l.litigationProfile?.location ?? "—"} · {l.litigationProfile?.activeCaseCount ?? 0} {t("active case(s)")}
                  </p>
                </div>
                <button
