  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-5 py-10 text-(--text)">
      <div className="w-full max-w-md rounded-2xl border p-8 text-center"
        ...
        <p className="text-xs font-bold uppercase tracking-widest text-(--accent) mb-2">Almost there</p>
        <h1 className="text-2xl font-bold leading-tight">Awaiting role assignment</h1>
        <p className="mt-3 text-sm text-(--muted) leading-relaxed">
          You&apos;re signed in as <span className="font-semibold text-(--text)">{session.name}</span>
          {displayEmail && <span className="block text-xs mt-1 font-mono">{displayEmail}</span>}
          . A superadmin needs to assign you a role before you can access the dashboard.
        </p>
        ...
        <div className="mt-6 rounded-xl px-4 py-3 text-left text-xs leading-relaxed"
          ...
          What happens next: a superadmin will see your name on their pending sign-ups list and pick the right role for you (litigation, social worker, HR, finance, etc.). Refresh this page once that&apos;s done — you&apos;ll be sent to your dashboard automatically.
        </div>
