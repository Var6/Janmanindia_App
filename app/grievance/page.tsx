...
export default async function GrievancePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-(--text)">{t("Grievance Box")}</h1>
        <p className="text-sm text-(--muted) mt-1">
          {t("A direct, confidential channel to HR. Every grievance is logged, tracked, and answered.")}
        </p>
      </div>
      ...
      <section>
        <h2 className="text-lg font-semibold text-(--text) mb-3">{t("My Submissions")}</h2>
      ...
      {isHr && (
        <div className="p-4 rounded-xl border border-(--border) bg-(--surface)">
          <p className="text-xs text-(--muted)">
            {t("Staff view — open the")}{" "}
            <a href={session.role === "hr" ? "/hr/grievances" : "/director/grievances"}
              className="font-semibold underline" style={{ color: "var(--accent)" }}>
              {t("Grievance Inbox")}
            </a>{" "}
            {t("to review and respond to all submissions.")}
          </p>
        </div>
      )}
    </div>
