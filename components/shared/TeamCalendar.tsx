...
/**
 * Org-wide agenda: every staff member's activities, appointments, trainings and
 * case hearings across a 7-day-back → 30-day-forward window, plus a per-staff
 * load summary. Used by both the Director and Administrator calendar pages.
 *
 * @param assignHref   optional "assign new task" CTA target (role-specific)
 * @param caseHrefBase optional base path for hearing links (e.g. /director/cases);
 *                     when omitted, hearings render as plain (non-clickable) rows.
 */
export default async function TeamCalendar({
  assignHref,
  caseHrefBase,
}: {
  role: string;
  ...
  for (const tr of trainingsRaw) {
    const at = new Date(tr.date);
    const k = dayKey(at);
    if (!byDay.has(k)) byDay.set(k, []);
    byDay.get(k)!.push({ kind: "training", at, training: tr });
  }
  ...
  // Build the day list — every day in the window, even if empty.
                      ...
                      {items.length === 0 ? (
                        ...
                        items.map((it, i) => {
                          ...
                          if (it.kind === "hearing") {
                            ...
                            const hearingStyle = { borderColor: "color-mix(in srgb, var(--error) 35%, var(--border))", background: "var(--bg)" };
                            return caseHrefBase ? (
                              <Link key={`hr-${i}-${String(c._id)}`} href={`${caseHrefBase}/${String(c._id)}`}
                                className="rounded-lg border px-3 py-2 text-xs block transition-colors hover:border-(--accent)"
                                style={hearingStyle}>
                                {hearingInner}
                              </Link>
                            ) : (
                              <div key={`hr-${i}-${String(c._id)}`} className="rounded-lg border px-3 py-2 text-xs block"
                                style={hearingStyle}>
                                {hearingInner}
                              </div>
                            );
