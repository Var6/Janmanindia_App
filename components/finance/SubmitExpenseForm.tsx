  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border p-5 space-y-3"
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-base font-bold text-(--text)">{t("Submit an expense")}</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <select name="projectId" required defaultValue=""
          ...
          <option value="" disabled>{t("Project…")}</option>
          {projects.map(p => (
            <option key={p._id} value={p._id}>
              {p.code} — {p.name}{typeof p.remaining === "number" ? ` · ₹${p.remaining.toLocaleString("en-IN")} ${t("left")}` : ""}
            </option>
          ))}
        </select>
        <select name="category" required defaultValue=""
          ...
          {visibleCats.map(c => <option key={c.value} value={c.value} title={t(c.hint)}>{t(c.label)}</option>)}
        </select>
      </div>

      <input name="title" required maxLength={200} placeholder={t("Short title (e.g. AC repair – Patna office)")}
        className="w-full px-3 py-2 rounded-lg border text-sm"
        style={{ background: "var(--bg)", borderColor: "var(--border)", color: "var(--text)" }} />
      <textarea name="description" rows={2} placeholder={t("Describe what you bought / did")}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <input name="amount" required type="number" min={1} step="0.01" placeholder={t("Amount (₹)")}
          className="px-3 py-2 rounded-lg border text-sm"
          style={{ background: "var(--bg)", borderColor: "var(--border)", color: "var(--text)" }} />
        <input name="vendor" placeholder={t("Vendor (optional)")}
          className="px-3 py-2 rounded-lg border text-sm"
          style={{ background: "var(--bg)", borderColor: "var(--border)", color: "var(--text)" }} />
        <input name="incurredAt" type="date" placeholder={t("Date incurred")}
          className="px-3 py-2 rounded-lg border text-sm"
          style={{ background: "var(--bg)", borderColor: "var(--border)", color: "var(--text)" }} />
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {receiptUrl ? (
          ...
          <button type="button" onClick={() => fileRef.current?.click()} disabled={uploadingReceipt}
            ...
            {uploadingReceipt ? t("Uploading…") : `📎 ${t("Attach receipt")}`}
          </button>
        )}
      </div>
