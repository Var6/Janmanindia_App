 ...
 *  (Central + Bihar) + laws + PLV training with quizzes + PLV join + refer. */
...
export default function JanSahayakCitizen() {
  ...
  useEffect(() => { lsGet("jsc_s").then((d) => { if (d) setSubs(d); }); lsGet("jsc_p").then((d) => { if (d) setPlvSubs(d); }); }, []);
  ...
  async function doSubmit() {
    ...
    let parsed = { type: "legal", urgency: "normal", summary: "Received", hi_summary: "प्राप्त", issues: [], next_steps: ["Team will call within 24 hours"], hi_next_steps: ["टीम 24 घंटे में कॉल करेगी"] };
    try {
      const raw = await claudeCall("", [{ role: "user", content: p }], 800);
      try { parsed = JSON.parse(raw.replace(/```json|```/g, "").trim()); } catch (e) { /* keep default */ }
    } catch (e) { /* keep default */ }
    setTriage(parsed);
    const sub = { id: uid(), name: form.name, phone: form.phone, location: form.location, district: form.district, issue: form.issue, type: parsed.type, urgency: parsed.urgency, date: new Date().toISOString().slice(0, 10), status: "pending" };
    const upd = [...subs, sub]; setSubs(upd); await lsSet("jsc_s", upd);
    setStep(3); setLoading(false);
  }

  async function doCampaign() {
    setCampLoad(true); setCampOut("");
    const ins = MOVEMENTS.find((m) => m.name === campForm.movement);
    const sk = Object.entries(campForm.stakeholders).filter((e) => e[1]).map((e) => e[0] + ": " + e[1]).join(", ");
    ...
    try { setCampOut(await claudeCall("", [{ role: "user", content: p }], 1500)); } catch (e) { setCampOut("Error generating campaign. Please try again."); }
  ...
  async function doPlv() {
    ...
    const upd = [...plvSubs, sub]; setPlvSubs(upd); await lsSet("jsc_p", upd);
    setScreen("plvOk");
  }

  const fellow = FELLOWS.find((f) => f.dist === form.district);
  const schemes = (schTab === "central" ? CENTRAL : BIHAR).filter((s) => { if (!schQ) return true; const q = schQ.toLowerCase(); return s.name.toLowerCase().includes(q) || s.hi.includes(q) || s.cat.toLowerCase().includes(q); });
  const laws = LAWS.filter((l) => { if (!lawQ) return true; const q = lawQ.toLowerCase(); return l.name.toLowerCase().includes(q) || l.hi.includes(q) || l.cat.toLowerCase().includes(q); });
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg,#8B1A0A 0%,#C84B31 50%,#E8835C 100%)", fontFamily: "'Noto Sans',sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "36px 20px 60px" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 27, fontWeight: 700, color: "#fff", marginBottom: 3 }}>{tx("Jan Sahayak", "जन सहायक")}</h1>
        ...
        <div style={{ background: "rgba(255,255,255,0.12)", borderRadius: 12, padding: "11px", marginBottom: 14, display: "flex", justifyContent: "center", gap: 8 }}>
          {["hi", "en"].map((l) => <button key={l} onClick={() => setLang(l)} style={{ padding: "7px 20px", borderRadius: 9, border: "none", background: lang === l ? "#fff" : "transparent", color: lang === l ? "#C84B31" : "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{l === "hi" ? "हिंदी" : "English"}</button>)}
        </div>
        {[
          { icon: "🆘", label: tx("Get Help Now", "अभी मदद लें"), sub: tx("Legal issue, FIR, rights violation", "कानूनी समस्या, FIR, अधिकार उल्लंघन"), fn: () => { setScreen("intake"); setStep(1); }, primary: true },
          { icon: "📢", label: tx("Design a Campaign", "अभियान बनाएं"), sub: tx("AI-powered awareness & advocacy campaign", "AI की मदद से जागरूकता अभियान"), fn: () => { setScreen("campaign"); setCampStep(1); setCampOut(""); }, highlight: true },
          { icon: "📋", label: tx("Government Schemes", "सरकारी योजनाएं"), sub: tx("Central + Bihar — eligibility, how to apply", "केंद्र + बिहार की योजनाएं"), fn: () => setScreen("schemes") },
          { icon: "⚖", label: tx("Know Your Rights", "अपने अधिकार जानें"), sub: tx("Acts, laws, landmark cases, FAQs", "कानून, अधिनियम, महत्वपूर्ण मामले"), fn: () => setScreen("laws") },
          { icon: "📚", label: tx("PLV Training", "PLV प्रशिक्षण"), sub: tx("Learn and teach rights in your community", "समुदाय में अधिकार सीखें और सिखाएं"), fn: () => setScreen("plvTrain") },
          { icon: "🤝", label: tx("Join as PLV Volunteer", "PLV बनें"), sub: tx("Register for paralegal training — free", "पैरालीगल प्रशिक्षण के लिए पंजीकरण — मुफ्त"), fn: () => setScreen("plvJoin") },
          { icon: "📨", label: tx("Refer Someone as PLV", "किसी को PLV रेफर करें"), sub: tx("Your referral code: " + refCode, "आपका कोड: " + refCode), fn: () => setScreen("plvRefer") },
        ].map((item, i) => (
  ...
  if (screen === "campaign") return (
    <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif" }}>
      {Hdr(tx("Design a Campaign", "अभियान बनाएं"), () => setScreen("home"))}
      <div style={{ display: "flex", margin: "12px 18px 0" }}>{[1, 2, 3].map((i) => <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i <= campStep ? W.purple : W.border, marginRight: i < 3 ? 4 : 0 }} />)}</div>
      <div style={S}>
        {campStep === 1 && <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <Fi label={tx("Your name", "आपका नाम")} value={campForm.contactName} onChange={(e) => setCampForm((p) => ({ ...p, contactName: e.target.value }))} />
            <Fi label={tx("Your phone", "आपका फोन")} type="tel" value={campForm.contactPhone} onChange={(e) => setCampForm((p) => ({ ...p, contactPhone: e.target.value }))} />
          </div>
          <Btn onClick={() => setCampStep(2)} disabled={!campForm.issue.trim() || !campForm.location.trim()}>{tx("Next →", "अगला →")}</Btn>
        </div>}
        {campStep === 2 && <div>
          ...
          {ART_FORMATS.map((af) => (
            <button key={af.name} onClick={() => setCampForm((p) => ({ ...p, artFormat: af.name }))} style={{ width: "100%", textAlign: "left", padding: "11px 13px", borderRadius: 11, border: "2px solid " + (campForm.artFormat === af.name ? W.purple : W.border), background: campForm.artFormat === af.name ? "rgba(106,27,154,0.07)" : "#fff", marginBottom: 8, cursor: "pointer", display: "flex", gap: 12 }}>
          ...
          <Fs label="" value={campForm.movement} onChange={(e) => setCampForm((p) => ({ ...p, movement: e.target.value }))} options={[{ value: "", label: tx("— Choose a movement —", "— आंदोलन चुनें —") }, ...MOVEMENTS.map((m) => ({ value: m.name, label: m.name + " (" + m.theme + ")" }))]} />
          {campForm.movement && (() => { const m = MOVEMENTS.find((x) => x.name === campForm.movement); return m ? <div style={{ background: "rgba(106,27,154,0.07)", borderRadius: 9, padding: "9px 12px", marginBottom: 10, fontSize: 12, color: W.dim }}>{m.tools.map((t, i) => <div key={i}>• {t}</div>)}</div> : null; })()}
          ...
          {PSTAKE.map((r) => (
            <div key={r.k} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 7 }}>
              ...
              <input value={campForm.stakeholders[r.k] || ""} onChange={(e) => { const v = e.target.value; setCampForm((p) => { const sk = Object.assign({}, p.stakeholders); sk[r.k] = v; return Object.assign({}, p, { stakeholders: sk }); }); }} placeholder={tx("Name", "नाम")} style={{ flex: 1, border: "1px solid " + W.border, borderRadius: 8, padding: "7px 10px", fontSize: 12, color: W.text }} />
            </div>
          ))}
          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
            <button onClick={() => setCampStep(1)} style={{ flex: 1, padding: "12px", borderRadius: 10, border: "2px solid " + W.border, background: "transparent", color: W.dim, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>← {tx("Back", "वापस")}</button>
        ...
        {campStep === 3 && <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {campOut && <button onClick={() => navigator.clipboard && navigator.clipboard.writeText(campOut)} style={{ padding: "6px 11px", borderRadius: 7, border: "none", background: W.purple, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{tx("Copy", "कॉपी")}</button>}
              <button onClick={() => { setCampStep(1); setCampOut(""); }} style={{ padding: "6px 11px", borderRadius: 7, border: "1px solid " + W.border, background: "transparent", color: W.dim, fontSize: 11, cursor: "pointer" }}>{tx("New", "नया")}</button>
            </div>
          </div>
  ...
  if (screen === "intake") return (
    <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif" }}>
      {Hdr(tx("Get Help", "मदद लें"), () => { if (step === 1) setScreen("home"); else setStep(step - 1); })}
      <div style={{ display: "flex", margin: "12px 18px 0" }}>{[1, 2].map((i) => <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i <= step ? W.accent : W.border, marginRight: i < 2 ? 4 : 0 }} />)}</div>
      <div style={S}>
        {step === 1 && <div>
          ...
          <Btn onClick={() => setStep(2)} disabled={!form.name.trim() || !form.phone.trim()}>{tx("Next →", "अगला →")}</Btn>
          <div style={{ padding: "9px 12px", background: W.accentSoft, borderRadius: 9, marginTop: 9, fontSize: 11, color: W.dim }}>🔒 {tx("Only shared with your assigned lawyer or social worker.", "केवल आपके वकील या सामाजिक कार्यकर्ता के साथ साझा।")}</div>
        </div>}
        {step === 2 && <div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: W.text, marginBottom: 3 }}>{tx("Your Problem", "आपकी समस्या")}</h2>
        ...
        {step === 3 && triage && <div>
          <div style={{ textAlign: "center", marginBottom: 16 }}><div style={{ fontSize: 50, marginBottom: 9 }}>{triage.urgency === "high" ? "🚨" : "✓"}</div><h2 style={{ fontSize: 21, fontWeight: 700, color: W.text, marginBottom: 3 }}>{tx("Thank You", "धन्यवाद")}</h2><p style={{ fontSize: 12, color: W.dim }}>{tx("Your request has been received.", "आपका अनुरोध प्राप्त हो गया।")}</p></div>
          <div style={{ background: triage.type === "legal" ? W.accentSoft : "rgba(46,125,50,0.08)", borderRadius: 12, padding: "14px 16px", marginBottom: 10, border: "2px solid " + (triage.type === "legal" ? W.accent : W.green) }}>
            ...
            {triage.issues && triage.issues.map((x, i) => <div key={i} style={{ fontSize: 11, color: W.dim, paddingTop: 3 }}>• {x}</div>)}
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 12, padding: "12px 15px", marginBottom: 12, border: "1px solid " + W.border }}>
            ...
            {(hi ? triage.hi_next_steps : triage.next_steps).map((s, i) => <div key={i} style={{ fontSize: 13, color: W.text, padding: "3px 0", display: "flex", gap: 7 }}><span style={{ color: W.accent, fontWeight: 700, flexShrink: 0 }}>{i + 1}.</span>{s}</div>)}
          </div>
          <Btn onClick={() => { setScreen("home"); setStep(1); setForm({ name: "", phone: "", location: "", district: "Purnia", issue: "", issueType: "legal" }); setTriage(null); }}>← {tx("Back to Home", "होम पर वापस")}</Btn>
        </div>}
      </div>
    </div>
  );
  ...
  if (screen === "schemes") return (
    <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif" }}>
      {Hdr(tx("Government Schemes", "सरकारी योजनाएं"), () => setScreen("home"))}
      <div style={S}>
        <input value={schQ} onChange={(e) => setSchQ(e.target.value)} placeholder={tx("Search schemes...", "योजनाएं खोजें...")} style={{ width: "100%", border: "2px solid " + W.border, borderRadius: 10, padding: "10px 13px", fontSize: 14, color: W.text, marginBottom: 10, boxSizing: "border-box" }} />
        ...
        {schemes.map((s) => (
          <details key={s.id} style={{ background: "#fff", borderRadius: 12, marginBottom: 9, border: "1px solid " + W.border, overflow: "hidden" }}>
            ...
            <div style={{ padding: "0 15px 13px", borderTop: "1px solid " + W.border }}>
              ...
              {s.docs.map((d, i) => <div key={i} style={{ fontSize: 12, color: W.text, padding: "2px 0", display: "flex", gap: 5 }}><span style={{ color: W.green }}>✓</span>{d}</div>)}
              ...
              <button onClick={() => { setForm((p) => ({ ...p, issue: tx("I need help with scheme: ", "इस योजना में मदद चाहिए: ") + (hi ? s.hi : s.name) })); setScreen("intake"); setStep(2); }} style={{ width: "100%", marginTop: 9, padding: "9px", borderRadius: 9, border: "none", background: W.accent, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{tx("Get Help →", "मदद लें →")}</button>
            </div>
          </details>
        ))}
      </div>
  ...
  if (screen === "laws") return (
    <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif" }}>
      {Hdr(tx("Know Your Rights", "अपने अधिकार जानें"), () => { if (activeLaw) { setActiveLaw(null); } else { setScreen("home"); } })}
      <div style={S}>
        {!activeLaw && <div>
          <input value={lawQ} onChange={(e) => setLawQ(e.target.value)} placeholder={tx("Search laws...", "कानून खोजें...")} style={{ width: "100%", border: "2px solid " + W.border, borderRadius: 10, padding: "10px 13px", fontSize: 14, color: W.text, marginBottom: 12, boxSizing: "border-box" }} />
          {laws.map((l) => (
            <button key={l.id} onClick={() => setActiveLaw(l)} style={{ width: "100%", textAlign: "left", padding: "12px 14px", borderRadius: 12, border: "1px solid " + W.border, background: "#fff", marginBottom: 8, cursor: "pointer", display: "flex", gap: 10 }}>
        ...
        {activeLaw && <div>
          <button onClick={() => setActiveLaw(null)} style={{ background: "none", border: "none", color: W.accent, fontSize: 13, fontWeight: 700, cursor: "pointer", marginBottom: 11, padding: "3px 0" }}>← {tx("Back", "वापस")}</button>
          <div style={{ background: "#fff", borderRadius: 12, padding: "15px 17px", marginBottom: 10, border: "1px solid " + W.border }}>
            ...
            <h2 style={{ fontSize: 17, fontWeight: 700, color: W.text, marginBottom: 7, lineHeight: 1.4 }}>{hi ? activeLaw.hi : activeLaw.name}</h2>
  ...
  if (screen === "plvTrain") {
    if (activeMod) return (
      <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif" }}>
        {Hdr(hi ? activeMod.hi : activeMod.title, () => { setActiveMod(null); setQuizAns(null); })}
        <div style={S}>
          ...
          <div style={{ background: W.accentSoft, borderRadius: 12, padding: "15px 17px", border: "1px solid " + W.accent + "30" }}>
            ...
            {activeMod.quiz.map((q, qi) => (
              <div key={qi}>
                ...
                {q.opts.map((opt, oi) => { const correct = oi === q.ans; const sel = quizAns === oi; const show = quizAns !== null; let bg = "#fff"; let brd = "1px solid " + W.border; let col = W.text; if (show && correct) { bg = "rgba(46,125,50,0.1)"; brd = "2px solid " + W.green; col = W.green; } else if (show && sel && !correct) { bg = "rgba(200,75,49,0.1)"; brd = "2px solid " + W.accent; col = W.accent; } return <button key={oi} onClick={() => { if (quizAns === null) setQuizAns(oi); }} style={{ width: "100%", textAlign: "left", padding: "10px 13px", borderRadius: 9, border: brd, background: bg, color: col, fontSize: 13, cursor: quizAns === null ? "pointer" : "default", marginBottom: 6, fontWeight: correct && show ? 700 : 400 }}>{show && correct ? "✓ " : show && sel && !correct ? "✗ " : ""}{opt}</button>; })}
                {quizAns !== null && <div style={{ padding: "9px 12px", background: "rgba(46,125,50,0.1)", borderRadius: 9, fontSize: 13, color: W.green, marginTop: 3 }}>{quizAns === q.ans ? tx("Correct! Well done!", "सही उत्तर! शाबाश!") : tx("Correct answer: ", "सही उत्तर: ") + q.opts[q.ans]}</div>}
              </div>
            ))}
          </div>
          <Btn onClick={() => { setActiveMod(null); setQuizAns(null); }}>{tx("← Back to Modules", "← मॉड्यूल पर वापस")}</Btn>
        </div>
      </div>
    );
    return (
      <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif" }}>
        {Hdr(tx("PLV Training", "PLV प्रशिक्षण"), () => setScreen("home"))}
        <div style={S}>
          ...
          {PLV_MOD.map((m) => (
            <button key={m.id} onClick={() => { setActiveMod(m); setQuizAns(null); }} style={{ width: "100%", textAlign: "left", padding: "13px 15px", borderRadius: 12, border: "1px solid " + W.border, background: "#fff", marginBottom: 8, cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 23, flexShrink: 0 }}>{m.icon}</span>
              <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 13, color: W.text }}>{hi ? m.hi : m.title}</div><div style={{ fontSize: 11, color: W.dim, marginTop: 2 }}>{tx("Tap to learn · Quiz included", "सीखने के लिए दबाएं · प्रश्न भी हैं")}</div></div>
  ...
  if (screen === "plvJoin") return (
    <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif" }}>
      {Hdr(tx("Join as PLV Volunteer", "PLV बनें"), () => setScreen("home"))}
      <div style={S}>
        ...
        <Fi label={tx("Full Name", "पूरा नाम")} value={plvForm.name} onChange={(e) => setPlvForm((p) => ({ ...p, name: e.target.value }))} />
        <Fi label={tx("Phone Number", "मोबाइल नंबर")} type="tel" value={plvForm.phone} onChange={(e) => setPlvForm((p) => ({ ...p, phone: e.target.value }))} placeholder="9876543210" />
        <Fi label={tx("Village / Area", "गाँव / मोहल्ला")} value={plvForm.location} onChange={(e) => setPlvForm((p) => ({ ...p, location: e.target.value }))} />
        <Fs label={tx("District", "जिला")} value={plvForm.district} onChange={(e) => setPlvForm((p) => ({ ...p, district: e.target.value }))} options={BD} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <Fi label={tx("Age", "उम्र")} type="number" value={plvForm.age} onChange={(e) => setPlvForm((p) => ({ ...p, age: e.target.value }))} />
          <Fi label={tx("Education", "शिक्षा")} value={plvForm.education} onChange={(e) => setPlvForm((p) => ({ ...p, education: e.target.value }))} placeholder={tx("e.g. 10th pass", "10वीं पास")} />
        </div>
        <Fi label={tx("Why do you want to become a PLV?", "PLV क्यों बनना चाहते हैं?")} value={plvForm.motivation} onChange={(e) => setPlvForm((p) => ({ ...p, motivation: e.target.value }))} rows={2} />
        <Fi label={tx("Referred by (name or code — optional)", "रेफर किया — वैकल्पिक")} value={plvForm.referredBy} onChange={(e) => setPlvForm((p) => ({ ...p, referredBy: e.target.value }))} />
  ...
  if (screen === "plvOk") return (
    <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ textAlign: "center", maxWidth: 360 }}>
        ...
        <h2 style={{ fontSize: 22, fontWeight: 700, color: W.text, marginBottom: 6 }}>{tx("Application Received!", "आवेदन प्राप्त हुआ!")}</h2>
        ...
        <Btn onClick={() => setScreen("home")}>← {tx("Back to Home", "होम पर वापस")}</Btn>
      </div>
    </div>
  );
  ...
  if (screen === "plvRefer") return (
    <div style={{ minHeight: "100vh", background: W.bg, fontFamily: "'Noto Sans',sans-serif" }}>
      {Hdr(tx("Refer Someone as PLV", "PLV के लिए रेफर करें"), () => setScreen("home"))}
      <div style={S}>
        <div style={{ background: W.accentSoft, borderRadius: 12, padding: "16px", marginBottom: 13, textAlign: "center", border: "1px solid " + W.accent + "33" }}>
          ...
          <button onClick={() => navigator.clipboard && navigator.clipboard.writeText("Join Jan Sahayak PLV Programme — code: " + refCode + " | Janman Peoples Foundation")} style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: W.accent, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{tx("Copy & Share", "कॉपी और शेयर करें")}</button>
        </div>
