...
var nav=isSuper?[{id:"home",ic:"\u{1F3EF}",lb:t("Command")},{id:"queue",ic:"⚡",lb:t("Queue")},{id:"map",ic:"\u{1F5FA}️",lb:t("Map")},{id:"legal",ic:"⚖️",lb:t("Legal")},{id:"docs",ic:"\u{1F4DC}",lb:t("Docs")}]:[{id:"home",ic:"\u{1F3EF}",lb:t("Home")},{id:"queue",ic:"⚡",lb:t("Queue")},{id:"cases",ic:"\u{1F4C1}",lb:t("My Cases")},{id:"legal",ic:"⚖️",lb:t("Legal")},{id:"docs",ic:"\u{1F4DC}",lb:t("Docs")}];
...
<style>{"@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@600;700;800&family=Mukta:wght@400;500;600;700;800&family=Playfair+Display:wght@700;900&display=swap');*{box-sizing:border-box;margin:0;padding:0}body{margin:0;background:"+P.bg+"}@keyframes rise{from{opacity:0;transform:translateY(8px)}to{opacity:1}}@keyframes breathe{0%,100%{opacity:1}50%{opacity:.45}}input,textarea,select{font-family:"+FN+";color:"+P.ink+";background:"+P.sf+";border:1px solid "+P.bd+";border-radius:7px;padding:9px 11px;font-size:12px;outline:none;width:100%}input:focus,textarea:focus,select:focus{border-color:"+P.ac+"}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:"+P.ac+"30;border-radius:4px}"}</style>
  ...
  {pg==="home"&&<div style={{animation:"rise .35s"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 2px 8px"}}>
      <div><div style={{fontSize:10,color:P.dim,letterSpacing:1}}>{t("WELCOME")}</div><div style={{fontFamily:HD,fontSize:17,fontWeight:800,color:P.gold}}>{me.n.split(" ")[0]}</div></div>
    ...
    <Card pat={true} s={{background:"linear-gradient(135deg,#221A11 0%,#1A140D 100%)",borderLeft:"3px solid "+P.gold}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{fontFamily:HD,fontSize:14,fontWeight:800,color:P.gold,letterSpacing:0.5}}>{"\u{1F305}"} {t("Daily Briefing")}</div><Btn sm={true} c={P.tl} dis={briefL} onClick={dailyBriefing}>{briefL?"...":briefing?t("Refresh"):t("Generate")}</Btn></div>
      {briefing?<pre style={{fontSize:11,lineHeight:1.7,whiteSpace:"pre-wrap",fontFamily:FN,color:P.ink}}>{briefing}</pre>:<div style={{fontSize:10,color:P.dim,fontStyle:"italic"}}>{t("Tap Generate for today's AI-powered priorities · situation, top 3, risks")}</div>}
    </Card>

    <Hd sub={t("Smart-ranked by urgency, SLA breach, days since action")} right={<Btn sm={true} ghost={true} c={P.ac} onClick={function(){setPg("queue");}}>{t("All")} {"→"}</Btn>}>{t("Priority Queue")}</Hd>
    {priorityQueue().slice(0,5).map(function(q,idx){return (<button key={q.id} onClick={function(){setSelQ(q);setPg("qdet");}} style={{width:"100%",textAlign:"left",padding:"10px 12px",borderRadius:9,border:"1px solid "+P.bd,background:P.cd,cursor:"pointer",fontFamily:FN,marginBottom:6,display:"flex",gap:10,alignItems:"flex-start",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}><span style={{fontSize:11,fontWeight:800,color:P.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:220}}>{q.qi} {q.qt==="incident"?q.type:q.scheme||q.type||t("Query")}</span><Pill c={SC[q.status||"new"]}>{q.status||"new"}</Pill></div>
    ...
    {priorityQueue().length===0&&<Card><div style={{textAlign:"center",padding:20,color:P.dim,fontSize:11}}>{t("All clear. No active priorities.")}</div></Card>}

    {isSuper?(
      <div style={{marginTop:14}}>
        <Hd sub={t("Live caseload across 6 districts")} right={<Btn sm={true} ghost={true} c={P.ac} onClick={function(){setPg("map");}}>{t("Full")} {"→"}</Btn>}>{t("District Dashboard")}</Hd>
        <Card>
          ...
          <div style={{display:"flex",justifyContent:"space-around",marginTop:6,fontSize:8,color:P.dim}}>
            <span><span style={{display:"inline-block",width:7,height:7,borderRadius:4,background:P.cr,marginRight:3}}/>{t("Critical")}</span>
            <span><span style={{display:"inline-block",width:7,height:7,borderRadius:4,background:P.ac,marginRight:3}}/>{t("Warning")}</span>
            <span><span style={{display:"inline-block",width:7,height:7,borderRadius:4,background:P.grn,marginRight:3}}/>{t("Active")}</span>
            <span><span style={{display:"inline-block",width:7,height:7,borderRadius:4,background:P.dim,marginRight:3}}/>{t("Quiet")}</span>
          </div>
        </Card>
      </div>
    ):(
      <div style={{marginTop:14}}>
        <Hd sub={t("Your caseload")+" · "+myCases().length+" "+t("total")} right={<Btn sm={true} ghost={true} c={P.ac} onClick={function(){setPg("cases");}}>{t("All")} {"→"}</Btn>}>{t("My Cases")}</Hd>
        <div style={{display:"flex",gap:5,marginBottom:8}}>{[{l:t("Active"),v:myCases().filter(function(q){return q.status==="in_progress"||q.status==="assigned";}).length,c:P.ac},{l:t("Overdue"),v:myCases().filter(function(q){return priorityScore(q,aLog).sla==="breach"&&q.status!=="resolved";}).length,c:P.cr},{l:t("Done"),v:myCases().filter(function(q){return q.status==="resolved";}).length,c:P.grn}].map(function(s,i){return (<div key={i} style={{flex:1,background:P.cd,border:"1px solid "+P.bd,borderRadius:8,padding:"10px 6px",textAlign:"center"}}><div style={{fontSize:20,fontWeight:900,color:s.c}}>{s.v}</div><div style={{fontSize:9,color:P.dim,fontWeight:600}}>{s.l}</div></div>);})}</div>
        {myCases().slice(0,4).map(function(q){var ps=priorityScore(q,aLog);return (<button key={q.id} onClick={function(){setSelQ(q);setPg("qdet");}} style={{width:"100%",textAlign:"left",padding:8,borderRadius:7,border:"1px solid "+P.bd,background:P.cd,cursor:"pointer",fontFamily:FN,marginBottom:4,display:"flex",gap:7}}><span style={{fontSize:14}}>{q.qi}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:10,fontWeight:700,color:P.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{q.qt==="incident"?q.type:q.scheme||q.type}</div><div style={{fontSize:8,color:ps.sla==="breach"?P.cr:P.dim}}>{q.district} {"·"} {ps.days}d {ps.sla==="breach"?"· "+t("OVERDUE"):""}</div></div><Pill c={SC[q.status||"new"]}>{q.status||"new"}</Pill></button>);})}
        {myCases().length===0&&<Card><div style={{textAlign:"center",padding:16,color:P.dim,fontSize:11}}>{t("No cases assigned to you yet")}</div></Card>}
      </div>
    )}
  </div>}

  {pg==="queue"&&<div style={{animation:"rise .3s"}}>
    <Hd sub={t("Ranked")+" · "+priorityQueue().length+" "+t("active")}>{t("Priority Queue")}</Hd>
    {priorityQueue().map(function(q,idx){return (<button key={q.id} onClick={function(){setSelQ(q);setPg("qdet");}} style={{width:"100%",textAlign:"left",padding:"10px 12px",borderRadius:9,border:"1px solid "+P.bd,background:P.cd,cursor:"pointer",fontFamily:FN,marginBottom:6,display:"flex",gap:10,alignItems:"flex-start",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:"flex",gap:5,marginTop:3,fontSize:8,alignItems:"center"}}><span style={{color:P.dim}}>{q.district}</span><span style={{color:q._p.sla==="breach"?P.cr:q._p.sla==="warning"?P.ac:P.grn,fontWeight:700}}>{q._p.days}d</span>{q._p.sla==="breach"&&<Pill c={P.cr}>{t("BREACH")}</Pill>}{q.urgency==="critical"&&<Pill c={P.cr}>{t("CRIT")}</Pill>}<span style={{marginLeft:"auto",color:P.gold,fontSize:8}}>{q.assignedTo||"—"}</span></div>
      </div>
    </button>);})}
    {priorityQueue().length===0&&<Card><div style={{textAlign:"center",padding:20,color:P.dim}}>{t("Queue is empty")}</div></Card>}
  </div>}

  {pg==="map"&&isSuper&&!selDist&&<div style={{animation:"rise .3s"}}>
    <Hd sub={t("Tap any district to drill down")}>{t("District Intelligence")}</Hd>
    <Card pat={true}>
    ...
    {DISTS.map(function(d){var s=distStat(d.n);return (<button key={d.n} onClick={function(){setSelDist(d.n);}} style={{width:"100%",textAlign:"left",padding:"10px 12px",borderRadius:8,border:"1px solid "+P.bd,background:P.cd,cursor:"pointer",fontFamily:FN,marginBottom:5,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <div style={{display:"flex",gap:4}}><Pill c={P.ac}>{s.active} {t("active")}</Pill>{s.overdue>0&&<Pill c={P.cr}>{s.overdue} {t("over")}</Pill>}{s.crit>0&&<Pill c={P.cr}>{"\u{1F6A8}"}{s.crit}</Pill>}</div>
    </button>);})}
  </div>}
  {pg==="map"&&selDist&&(function(){var s=distStat(selDist);var dq=queries.filter(function(q){return q.district===selDist;}).map(function(q){return Object.assign({},q,{_p:priorityScore(q,aLog)});}).sort(function(a,b){return b._p.score-a._p.score;});return (<div style={{animation:"rise .25s"}}>
    <button onClick={function(){setSelDist(null);}} style={{background:"none",border:"none",color:P.ac,fontSize:11,cursor:"pointer",fontFamily:FN,fontWeight:700,marginBottom:6}}>{"←"} {t("All Districts")}</button>
  ...
  {pg==="cases"&&!isSuper&&<div style={{animation:"rise .3s"}}>
    ...
    {myCases().map(function(q){var ps=priorityScore(q,aLog);var logs=aLog[q.id]||[];return (<button key={q.id} onClick={function(){setSelQ(q);setPg("qdet");}} style={{width:"100%",textAlign:"left",padding:"10px 12px",borderRadius:8,border:"1px solid "+(ps.sla==="breach"?P.cr+"60":P.bd),background:P.cd,cursor:"pointer",fontFamily:FN,marginBottom:6,position:"relative"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",fontSize:8}}><span style={{color:P.dim}}>{q.district} {"·"} {logs.length} {t("actions")}</span><span style={{color:ps.sla==="breach"?P.cr:ps.sla==="warning"?P.ac:P.grn,fontWeight:700}}>{ps.days}d{ps.sla==="breach"?" "+t("OVERDUE"):""}</span></div>
    </button>);})}
    {myCases().length===0&&<Card><div style={{textAlign:"center",padding:20,color:P.dim,fontSize:11}}>{t("No cases assigned yet")}</div></Card>}
  </div>}

  {pg==="qdet"&&selQ&&(function(){var q=selQ;var logs=aLog[q.id]||[];var ps=priorityScore(q,aLog);return (<div style={{animation:"rise .25s"}}>
    <button onClick={function(){setSelQ(null);setPg(isSuper?"queue":"cases");}} style={{background:"none",border:"none",color:P.ac,fontSize:11,cursor:"pointer",fontFamily:FN,fontWeight:700,marginBottom:6}}>{"←"} {t("Back")}</button>
    <Card pat={true} s={{borderLeft:"3px solid "+SC[q.status||"new"]}}>
      ...
      <div style={{display:"flex",gap:5,marginTop:6,flexWrap:"wrap"}}><Pill c={ps.sla==="breach"?P.cr:ps.sla==="warning"?P.ac:P.grn}>{"⏱"} {ps.days}d</Pill>{ps.sla==="breach"&&<Pill c={P.cr}>{t("SLA BREACH")}</Pill>}{q.urgency==="critical"&&<Pill c={P.cr}>{t("CRITICAL")}</Pill>}{q.assignedTo&&<Pill c={P.ac}>{"→"} {q.assignedTo.split(" ")[0]}</Pill>}</div>
    </Card>

    <Card><div style={{fontSize:10,fontWeight:800,color:P.ac,marginBottom:5,letterSpacing:0.5}}>{t("STATUS & ASSIGN")}</div><div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{["assigned","in_progress","resolved","closed"].map(function(s){return (<button key={s} onClick={async function(){await uQ(q.id,{status:s});await addLog(q.id,{action:"Status → "+s});}} style={{padding:"5px 9px",borderRadius:5,border:"1px solid "+SC[s]+"50",background:(q.status||"new")===s?SC[s]+"30":"transparent",color:SC[s],fontSize:9,fontWeight:800,cursor:"pointer",fontFamily:FN}}>{s.replace("_"," ")}</button>);})}</div><select onChange={function(e){if(e.target.value){uQ(q.id,{assignedTo:e.target.value});addLog(q.id,{action:"Assigned to "+e.target.value});}}} value={q.assignedTo||""} style={{marginTop:5,fontSize:10}}><option value="">{t("Assign...")}</option>{TEAM.map(function(tm){return <option key={tm.n} value={tm.n}>{tm.n} ({tm.r})</option>;})}</select></Card>

    <Card s={{borderLeft:"3px solid "+P.cr+"60"}}><div style={{fontSize:10,fontWeight:800,color:P.cr,marginBottom:5,letterSpacing:0.5}}>{"\u{1F4E2}"} {t("ESCALATE")}</div><div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{[{r:"Grave offence",l:t("Grave offence")},{r:"Non-compliance",l:t("Non-compliance")},{r:"Victim at risk",l:t("Victim at risk")},{r:"Media attention",l:t("Media attention")}].map(function(x){return (<button key={x.r} onClick={function(){escQ(q.id,x.r);}} style={{padding:"4px 8px",borderRadius:4,border:"1px solid "+P.cr+"40",background:"transparent",color:P.cr,fontSize:9,fontWeight:700,cursor:"pointer",fontFamily:FN}}>{x.l}</button>);})}</div></Card>

    <Card><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}><span style={{fontSize:10,fontWeight:800,color:P.blu,letterSpacing:0.5}}>{"\u{1F916}"} {t("AI ANALYSIS")}</span><Btn sm={true} c={P.blu} dis={aiL} onClick={function(){runAI(q);}}>{aiL?"...":t("Analyze")}</Btn></div>{aiR&&!aiR.error&&<div style={{fontSize:10,lineHeight:1.5}}><div style={{display:"flex",gap:3,marginBottom:4,flexWrap:"wrap"}}><Pill c={aiR.priority==="critical"?P.cr:aiR.priority==="high"?P.ac:P.grn}>{aiR.priority}</Pill><Pill c={P.blu}>{aiR.category}</Pill>{aiR.sla_days&&<Pill c={P.ac}>SLA {aiR.sla_days}d</Pill>}</div>{(aiR.actions||[]).map(function(a,i){return (<div key={i} style={{padding:"4px 6px",background:P.sf,borderRadius:4,marginBottom:3,borderLeft:"2px solid "+P.grn,fontSize:10}}><b>{i+1}.</b> {a.action} <span style={{color:P.dim,fontSize:8}}>{a.assignee} · {a.days}d</span></div>);})}{(aiR.legal||[]).length>0&&<div style={{marginTop:4,fontSize:9}}><b style={{color:P.ac}}>{t("Legal")}:</b> <span style={{color:P.dim}}>{aiR.legal.join(", ")}</span></div>}{(aiR.schemes||[]).length>0&&<div style={{fontSize:9}}><b style={{color:P.tl}}>{t("Schemes")}:</b> <span style={{color:P.dim}}>{aiR.schemes.join(", ")}</span></div>}{aiR.risk&&<div style={{padding:5,background:P.cr+"12",borderRadius:4,color:P.cr,fontSize:9,marginTop:4}}>{"⚠️"} {aiR.risk}</div>}</div>}</Card>

    <Card><div style={{fontSize:10,fontWeight:800,color:P.grn,marginBottom:5,letterSpacing:0.5}}>{"\u{1F4DD}"} {t("ACTION LOG")} ({logs.length})</div><div style={{display:"flex",gap:3,marginBottom:5}}><input id="logI" placeholder={t("Record action taken...")} style={{flex:1,fontSize:10,padding:"6px 9px"}}/><Btn sm={true} c={P.grn} onClick={function(){var el=document.getElementById("logI");if(el&&el.value.trim()){addLog(q.id,{action:el.value});el.value="";}}}>{t("Log")}</Btn></div>{logs.slice().reverse().map(function(l,i){return (<div key={i} style={{padding:"4px 0",borderBottom:"1px solid "+P.bd,fontSize:9}}><div style={{fontWeight:700,color:P.ink}}>{l.action}</div><div style={{color:P.dim,fontSize:8,marginTop:1}}>{l.by} · {fmt(l.date)}</div></div>);})}{logs.length===0&&<div style={{fontSize:9,color:P.dim,fontStyle:"italic",padding:6}}>{t("No actions logged yet · accountability starts with logging")}</div>}</Card>

    <Card><div style={{fontSize:10,fontWeight:800,color:P.tl,marginBottom:5,letterSpacing:0.5}}>{"\u{1F4E8}"} {t("COMMUNICATE")}</div><div style={{display:"flex",gap:3,flexWrap:"wrap"}}><Btn sm={true} c={P.grn} onClick={function(){window.open("https://wa.me/91"+(q.phone||"")+"?text="+encodeURIComponent("Janman JNA - Your query "+q.id+": "+(q.desc?q.desc.slice(0,60):q.scheme||"")+"\nStatus: "+(q.status||"under review")+"\nAssigned: "+(q.assignedTo||"our team")),"_blank");addLog(q.id,{action:"WhatsApp follow-up sent"});}}>{t("WhatsApp")}</Btn><Btn sm={true} c={P.blu} onClick={function(){window.open("mailto:janman.justice@gmail.com?subject="+encodeURIComponent("[JNA] "+q.id),"_blank");}}>{t("Email")}</Btn><Btn sm={true} c={P.pur} onClick={function(){window.open("https://wa.me/?text="+encodeURIComponent("\u{1F6A8} JNA ALERT "+q.id+"\n"+(q.desc?q.desc.slice(0,80):q.scheme||"")+"\n"+(q.district||"")+"\n@"+(q.assignedTo||"team")),"_blank");}}>{t("Group")}</Btn></div></Card>

    <Card><div style={{fontSize:10,fontWeight:800,color:P.gold,marginBottom:5,letterSpacing:0.5}}>{"⚡"} {t("ONE-TAP ACTIONS")}</div><div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{["Fact-Finding","FIR Draft","RTI Draft","Bail Draft","Legal Camp","Compensation","RTPS","Court Filing"].map(function(a){return (<button key={a} onClick={async function(){await addLog(q.id,{action:"\u{1F680} Initiated: "+a});if((q.status||"new")==="new"||q.status==="assigned")await uQ(q.id,{status:"in_progress"});}} style={{padding:"5px 9px",borderRadius:5,border:"1px solid "+P.bd,background:P.sf,color:P.ink,fontSize:9,fontWeight:700,cursor:"pointer",fontFamily:FN}}>{t(a)}</button>);})}</div></Card>

    <Card><div style={{fontSize:10,fontWeight:800,color:P.pur,marginBottom:8,letterSpacing:0.5}}>{"\u{1F4CA}"} {t("CASE LIFECYCLE")}</div><div style={{display:"flex",justifyContent:"space-between",position:"relative"}}><div style={{position:"absolute",top:10,left:14,right:14,height:2,background:P.bd}}/>{STAGES.map(function(st,i){var curIdx=q.status==="resolved"?7:q.status==="in_progress"?3:q.status==="assigned"?2:q.status==="escalated"?5:1;var active=i<=curIdx;return (<div key={st} style={{position:"relative",zIndex:1,textAlign:"center"}}><div style={{width:22,height:22,borderRadius:11,background:active?P.ac:P.bd,border:"2px solid "+(active?P.gold:P.bd),margin:"0 auto",fontSize:9,color:active?"#1A140D":P.dim,fontWeight:900,display:"flex",alignItems:"center",justifyContent:"center"}}>{i+1}</div><div style={{fontSize:7,color:active?P.gold:P.dim,marginTop:2,fontWeight:700,width:40,lineHeight:1.1}}>{t(st)}</div></div>);})}</div></Card>
  </div>);})()}

  {pg==="legal"&&!selMod&&<div style={{animation:"rise .3s"}}>
    <Hd sub={t("Vidhi AI · 20 modules · latest judgments")}>{t("Legal Research")}</Hd>
    <Card pat={true} s={{borderLeft:"3px solid "+P.tl}}><div style={{fontSize:11,fontWeight:800,color:P.tl,marginBottom:5,letterSpacing:0.5}}>{"\u{1F52C}"} {t("VIDHI AI")}</div><input value={rQ} onChange={function(e){setRQ(e.target.value);}} placeholder={t("Research any legal question...")} style={{marginBottom:5}}/><Btn full={true} c={P.tl} dis={rL||!rQ.trim()} onClick={async function(){setRL(true);setRR(null);try{var text=await vidhiResearch("Vidhi AI for Bihar. Research: \""+rQ+"\". SCC Online, LiveLaw. BNS/BNSS 2023. Bihar-specific. Under 600 words.",1200);setRR(text||"No results.");}catch(e){setRR("Error: "+(e&&e.message||"failed"));}setRL(false);}}>{rL?t("Researching..."):t("Research with Vidhi")}</Btn>{rR&&<div style={{marginTop:6,padding:10,background:P.sf,borderRadius:6,fontSize:10,lineHeight:1.6,maxHeight:260,overflowY:"auto",whiteSpace:"pre-wrap"}}>{rR}</div>}</Card>
  ...
  {pg==="legal"&&selMod&&<div style={{animation:"rise .3s"}}>
    <button onClick={function(){setSelMod(null);setRR(null);}} style={{background:"none",border:"none",color:P.ac,fontSize:11,cursor:"pointer",fontFamily:FN,fontWeight:700,marginBottom:6}}>{"←"} {t("All")}</button>
    ...
    <Btn full={true} c={P.blu} dis={rL} onClick={async function(){setRL(true);setRR(null);try{var text=await vidhiResearch("Vidhi AI. LATEST (2024-2026) \""+selMod.t+"\". LiveLaw, SCC. Bihar-specific. Full SCC citations. Actionable.",1500);setRR(text||"No results.");}catch(e){setRR("Error");}setRL(false);}}>{rL?"...":t("Latest Judgments (2024-2026)")}</Btn>
  ...
  {pg==="docs"&&<div style={{animation:"rise .3s"}}>
    <Hd sub={t("Paste legal doc · AI extracts parties, sections, drafts")}>{t("Document AI")}</Hd>
    <Card><textarea value={docT} onChange={function(e){setDocT(e.target.value);}} rows={6} placeholder={t("Paste FIR, court order, petition, certificate...")} style={{resize:"vertical",lineHeight:1.5}}/><div style={{marginTop:6}}><Btn full={true} c={P.blu} dis={aiL||!docT.trim()} onClick={function(){runDoc(docT);}}>{aiL?t("Analyzing..."):t("Analyze")}</Btn></div></Card>
    {ocrR&&!ocrR.error&&<div>
      <Card><div style={{display:"flex",gap:4,marginBottom:5,flexWrap:"wrap"}}><Pill c={P.blu}>{ocrR.doc_type}</Pill></div>{ocrR.case_no&&<div style={{fontSize:11,marginBottom:3}}><b style={{color:P.ac}}>{t("Case")}:</b> {ocrR.case_no}</div>}{ocrR.court&&<div style={{fontSize:11,marginBottom:3}}><b style={{color:P.ac}}>{t("Court")}:</b> {ocrR.court}</div>}{ocrR.parties&&ocrR.parties.length>0&&<div style={{fontSize:11,marginBottom:3}}><b style={{color:P.ac}}>{t("Parties")}:</b> {ocrR.parties.join(" v. ")}</div>}{ocrR.sections&&ocrR.sections.length>0&&<div style={{fontSize:11,marginBottom:3}}><b style={{color:P.ac}}>{t("Sections")}:</b> {ocrR.sections.join(", ")}</div>}{ocrR.findings&&<div style={{background:P.sf,padding:8,borderRadius:5,fontSize:11,lineHeight:1.5,marginTop:5}}>{ocrR.findings}</div>}</Card>
      <Card><div style={{fontSize:10,fontWeight:800,color:P.grn,marginBottom:5}}>{"✍️"} {t("DRAFTING SUGGESTIONS")}</div>{(ocrR.suggestions||[]).map(function(s,i){return (<div key={i} style={{padding:"5px 7px",background:P.sf,borderRadius:4,marginBottom:3,borderLeft:"2px solid "+P.grn,fontSize:10,lineHeight:1.5}}>{i+1}. {s}</div>);})}</Card>
      <Card><div style={{fontSize:10,fontWeight:800,color:P.pur,marginBottom:5}}>{"\u{1F4DD}"} {t("NEXT STEPS")}</div>{(ocrR.next_steps||[]).map(function(s,i){return (<div key={i} style={{padding:"5px 7px",background:P.sf,borderRadius:4,marginBottom:3,borderLeft:"2px solid "+P.pur,fontSize:10}}>{"→"} {s}</div>);})}</Card>
    </div>}
    <Card><div style={{fontSize:10,fontWeight:800,color:P.ac,marginBottom:5}}>{"⚡"} {t("QUICK TEMPLATES")}</div><div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{["Bail","HC Writ","Letter SP/DM","Fact-Finding","Representation","RTPS","Compensation","Affidavit","RTI","Notice"].map(function(tpl){return (<button key={tpl} onClick={function(){setDocT("Draft a "+tpl+" for: [describe case]\n\nDistrict:\nParties:\nFacts:\nRelief sought:");}} style={{padding:"5px 9px",borderRadius:5,border:"1px solid "+P.bd,background:P.sf,color:P.ink,fontSize:9,fontWeight:700,cursor:"pointer",fontFamily:FN}}>{t(tpl)}</button>);})}</div></Card>
  </div>}
